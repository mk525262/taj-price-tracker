...
def save_history(member_price, standard_price):
    ...
    ws.append([
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"), CHECKIN, CHECKOUT,
        ADULTS, ROOMS, COMPOSITIONS, HOTEL_NAME, member_price, standard_price,
...
def main():
    with sync_playwright() as p:
        ...
        target_url = (
            HOTEL_URL
            + f"?dateIn={CHECKIN}&nights={NIGHTS}&compositions={COMPOSITIONS}"
            + "&stayplus=false&snu=false&accessibleRooms=false&hideWDR=false"
            + "&productCode=null&hideHotelDetails=false"
        )
