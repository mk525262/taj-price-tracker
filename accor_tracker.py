...
def send_telegram(message):
    ...
    if not token: raise RuntimeError("TAJ_TELEGRAM_BOT_TOKEN secret is missing")
...
def save_history(member_price, standard_price):
    if HISTORY_FILE.exists(): wb = load_workbook(HISTORY_FILE); ws = wb.active
    else:
    ...
    wb.save(HISTORY_FILE); print(f"Excel history saved: {HISTORY_FILE}", flush=True)

def parse_displayed_price(text):
    clean = re.sub(r"\s+", " ", text.replace("\u00a0", " ")).strip(); amount = r"₹\s*([\d,]+(?:\.\d+)?)"
    for pattern in [rf"Member\s+rate\s+From\s*{amount}\s+Public\s+rate\s+from\s*{amount}", rf"Member\s+rate\s+{amount}\s+Public\s+rate\s+from\s*{amount}", rf"Member\s+rate.*?From\s*{amount}.*?Public\s+rate\s+from\s*{amount}"]:
...
def main():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True); context = browser.new_context(locale="en-IN")
        def route_graphql(route):
            req = route.request
            if "api.accor.com/bff/v1/graphql" not in req.url: route.continue_(); return
            raw = req.post_data or ""
            if "HotelPageHot" in raw:
                print("HOTELPAGEHOT REQUEST:", raw[:8000], flush=True)
        ...
        page.goto(target_url, wait_until="domcontentloaded", timeout=60000); print("FINAL URL:", page.url, flush=True)
        page.wait_for_timeout(18000)
        body = page.locator("body").inner_text(timeout=10000); clean = re.sub(r"\s+", " ", body.replace("\u00a0"," ")).strip()
        print("SEARCH STATE DEBUG:", clean[:3500], flush=True)
        prices = parse_displayed_price(body)
        if not prices: browser.close(); raise RuntimeError("Official Accor displayed member/public price was not found; refusing to send a possibly wrong price.")
        member_price, standard_price = prices
        print(f"OFFICIAL ACCOR MEMBER : ₹{member_price:,.0f} / stay", flush=True); print(f"OFFICIAL ACCOR STANDARD : ₹{standard_price:,.0f} / stay", flush=True)
        ...
        print("Telegram message sent.", flush=True); browser.close()
